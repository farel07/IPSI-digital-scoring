            ...
            <div class="row">
                ...
                <div class="col-4">
                    <div class="p-3 border bg-light text-dark text-center" style="border-radius: 10px; height: 100%;">
                        <h5 class="text-danger fw-bold">{{ $peserta1->player->name ?? 'MENUNGGU PESERTA' }}</h5>
                        <p class="m-auto">{{ $peserta1->player->contingent->name ?? 'Kontingen' }}</p>
                    </div>
                </div>
                {{-- Sudut Biru --}}
                <div class="col-4">
                    <div class="p-3 border bg-light text-dark text-center" style="border-radius: 10px; height: 100%;">
                        <h5 class="text-primary fw-bold">{{ $peserta2->player->name ?? 'MENUNGGU PESERTA' }}</h5>
                        <p class="m-auto">{{ $peserta2->player->contingent->name ?? 'Kontingen' }}</p>
                    </div>
                </div>
        ...
        async function handleRoundUpdate(roundNumber) {
            ...
            try {
                ...
                if (result.status === 'success') {
                    ...
                    // Update UI tombol
                    if (roundNumber == 2) {
                        round1Btn.classList.add('disabled');
                    } else if (roundNumber == 3) {
                        round1Btn.classList.add('disabled');
                        round2Btn.classList.add('disabled');
                    }
                } else {
